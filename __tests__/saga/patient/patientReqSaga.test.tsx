...
describe("test patientreqwatcherSaga", () => {
  ...
  it("test login loading", () => {
    expect(result.next().value).toEqual(
      takeLatest(patientReqActions.getPatientRequest, patientReqSaga)
    )
  })
...
describe("testing loginSaga", () => {
  ...
  it("should dispatch success action", async () => {
    ...
    expect(dispatched).toEqual(
      [patientReqActions.getPatientRequestSuccess(response.data)]
    );
    generator.mockClear();
  })
  it("should dispatch error action", async () => {
    ...
    expect(generator).toHaveBeenCalledTimes(1);
    expect(dispatched).toEqual(
      [patientReqActions.getPatientRequestFailure(undefined)]
    );
    generator.mockClear();
  })
})
